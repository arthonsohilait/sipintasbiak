    ...
    <div class="max-w-4xl mx-auto pb-12">
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1">Perbarui detail informasi untuk sektor unggulan ini.</p>
            </div>
        ...
        <div class="bg-blue-50/50 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-blue-100 overflow-hidden relative" x-data="{ isLoading: false }">
            ...
            <form action="{{ route('sectors.update', $sector->id) }}" method="POST" enctype="multipart/form-data" class="divide-y divide-blue-100" @submit="isLoading = true">
                @csrf
                @method('PUT')
                ...
                <div class="p-8 md:p-10 space-y-8">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $sector->name) }}" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3.5 px-4" required>
                    ...
                    <div>
                        ...
                        @if($sector->image)
                            <div class="mb-4">
                                <img src="{{ asset('storage/' . $sector->image) }}" class="h-44 w-auto rounded-2xl object-cover shadow-md border border-slate-100">
                            </div>
                        @endif
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-2xl group hover:border-komdigi-blue transition-colors bg-slate-50/50">
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-slate-500">Abaikan jika tidak ingin mengganti. Maks 2MB.</p>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="6" class="w-full rounded-xl border-slate-200 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3.5 px-4" required>{{ old('description', $sector->description) }}</textarea>
                ...
                <div class="p-8 bg-slate-50 flex items-center justify-between">
                    <p class="text-slate-500 text-sm italic font-medium">Data yang Anda perbarui akan langsung tampil di portal publik.</p>
