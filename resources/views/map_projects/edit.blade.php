<x-dashboard-layout>
    <div class="max-w-5xl mx-auto pb-12">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Data Pemetaan</h1>
                <p class="text-slate-500 text-sm mt-1">Perbarui informasi lokasi atau detail proyek strategis.</p>
            </div>
        ...
        <div class="bg-blue-50/50 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-blue-100 overflow-hidden relative" x-data="{ isLoading: false }">
            ...
            <form action="{{ route('map-projects.update', $mapProject->id) }}" method="POST" enctype="multipart/form-data" class="divide-y divide-blue-100" @submit="isLoading = true">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $mapProject->name) }}" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4" required>
                            ...
                            <div>
                                ...
                                <select name="sector" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4" required>
                                    ...
                                    @foreach($sectors as $sector)
                                        <option value="{{ $sector->name }}" {{ old('sector', $mapProject->sector) == $sector->name ? 'selected' : '' }}>
                                            {{ $sector->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                <textarea name="address" rows="2" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4" required>{{ old('address', $mapProject->address) }}</textarea>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 mb-2">Latitude <span class="text-rose-500">*</span></label>
                                    <input type="text" name="latitude" value="{{ old('latitude', $mapProject->latitude) }}" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4 font-mono" required>
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 mb-2">Longitude <span class="text-rose-500">*</span></label>
                                    <input type="text" name="longitude" value="{{ old('longitude', $mapProject->longitude) }}" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4 font-mono" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                <textarea name="description" rows="4" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4" required>{{ old('description', $mapProject->description) }}</textarea>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 mb-2">Kondisi Saat Ini</label>
                                    <textarea name="condition" rows="3" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4">{{ old('condition', $mapProject->condition) }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 mb-2">Peluang Investasi</label>
                                    <textarea name="investment_opportunity" rows="3" class="w-full rounded-xl bg-blue-50/30 border-slate-300 focus:ring-4 focus:ring-komdigi-blue/10 focus:border-komdigi-blue transition-all py-3 px-4">{{ old('investment_opportunity', $mapProject->investment_opportunity) }}</textarea>
                                </div>
                            </div>
                            <div>
                                ...
                                @if($mapProject->image)
                                    <div class="mb-4">
                                        <img src="{{ asset('storage/' . $mapProject->image) }}" class="h-40 w-auto rounded-2xl object-cover shadow-md border border-slate-100">
                                    </div>
                                @endif
                ...
                <div class="p-8 md:p-12 bg-slate-50 flex items-center justify-between">
                    <p class="text-slate-500 text-sm italic font-medium">Data diperbarui terakhir pada {{ $mapProject->updated_at->format('d/m/Y H:i') }}.</p>
